...
class BookingDetailsPlansRow extends GetView<BookingDetailsController> {
  final BranchModel selectedBranch;

  const BookingDetailsPlansRow({super.key, required this.selectedBranch});
    ...
    return GetBuilder<BookingDetailsController>(builder: (controller) {
      return SingleChildScrollView(
        ...
        child: Row(
          ...
          children: List.generate(selectedBranch.pricing?.length ?? 0, (index) {
            bool isSelected = controller.enrollmentType ==
                selectedBranch.pricing?[index].enrollmentType;
                      ...
                      ? ClipPath(
                          ...
                          child: Container(
                            ...
                            child: content(
                                ...
                                selectedBranch.pricing?[index].enrollmentType ??
                                    "",
                                selectedBranch.pricing?[index].priceAmount ??
                                    ""),
                          ),
                        )
                      : Container(
      ...
      children: [
        ...
        CustomText(
          ...
          textStyle: TextStyles.button12.copyWith(
              color: isSelected ? ColorCode.white : ColorCode.neutral500),
        ),
      ],
    );
  }
...
class CleanCouponClipper extends CustomClipper<Path> {
  ...
  Path getClip(Size size) {
    ...
    int horizontalCount =
        ((width - 2 * cornerRadius) / (cutWidth + cutDepth) / 1.5).floor();
    double horizontalSpacing =
        (width - 2 * cornerRadius - horizontalCount * cutWidth) /
            (horizontalCount + 1) *
            1.15;

    int verticalCount =
        ((height - 2 * cornerRadius) / (cutWidth + cutDepth) / 1.5).floor();
    double verticalSpacing =
        (height - 2 * cornerRadius - verticalCount * cutWidth) /
            (verticalCount + 1) *
            1.15;
    ...
    for (int i = 0; i < horizontalCount; i++) {
      double startX =
          width - cornerRadius - horizontalSpacing * (i + 1) - cutWidth * i;
    ...
    for (int i = 0; i < verticalCount; i++) {
      double startY =
          height - cornerRadius - verticalSpacing * (i + 1) - cutWidth * i;
