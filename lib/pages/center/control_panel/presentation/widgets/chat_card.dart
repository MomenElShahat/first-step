...
class ChatCard extends GetView<ControlPanelController> {
  final Contacts? chats;
  final ParentModel? parent;

  const ChatCard({super.key, this.chats, this.parent,});
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () async {
        ...
        await Get.toNamed(Routes.CHAT_DETAILS_SCREEN, arguments: {
          "receiverId": chats?.contactId.toString() ?? parent?.id.toString(),
          "name": chats?.contact?.name ?? parent?.name,
        });
                ...
                Stack(
                  ...
                  children: [
                    Container(
                      ...
                      child: Padding(
                        padding: EdgeInsets.only(top: controller.arabicRegex.hasMatch(chats != null
                            ? chats?.contact?.name ?? ""
                            : parent?.name ?? "") ? 0 : 5),
                        child: Center(
                            child: CustomText(
                                controller
                                    .getFirstTwoCapitalLetters(chats != null
                                        ? chats?.contact?.name ?? ""
                                            : parent?.name ?? ""),
                    ...
                    if(chats != null)
                      Container(
                        ...
                        decoration: BoxDecoration(
                          shape: BoxShape.circle,
                          color: chats?.isOnline == 1 ? ColorCode.info600 : ColorCode.neutral400,
